...
To learn more about calibration adb gauges parts, check out the [Calibration Parts](./calibration-parts) section.
...
Everything is printed ? Nice ! Let's place all the heat inserts, then!

Head over to the [Heat Inserts](./heat-inserts) section for the next steps of the build.

:nav-buttons{right="Placing the Heat Inserts" right-link="./heat-inserts" right-icon="i-lucide-chevron-right"}